...
// ---------- COLOR SEGÚN CUANTIL (AMARILLO / PÚRPURA) ----------
...
// ---------- FUNCIÓN PARA ACTUALIZAR ETIQUETAS INTELIGENTES ----------
function actualizarEtiquetas() {
    if (!map || !macroLayer || !distritoLayer) return;
    const zoom = map.getZoom();

    // 1. Macrodistritos: solo zoom <= 10 y sin distrito filtrado
        ...
        if (tooltip) {
            if (mostrarMacro) {
                ...
                if (seleccionMacro !== 'ALL' && nombre && normalize(nombre) === normalize(seleccionMacro)) {
                    tooltip._container.style.color = '#f59e0b';
                } else {
                    tooltip._container.style.color = '#3b82f6';
                }
            } else {
                layer.closeTooltip();
            }
    ...
    // 2. Distritos: zoom entre 11 y 13, o si hay distrito filtrado (zoom >= 11)
        ...
        if (tooltip) {
            if (mostrarDistrito) {
                ...
                if (seleccionDistrito !== 'ALL' && nombre && normalize(nombre) === normalize(seleccionDistrito)) {
                    ...
                    tooltip._container.style.color = '#10b981';
                }
            } else {
                layer.closeTooltip();
            }
    ...
    // 3. OTB: ya no se manejan aquí, ahora son solo al hover (sticky tooltip)
    // No hacemos nada, los tooltips se activan por mouseover
}
    ...
    if (targetLayer && foundFeature) {
        ...
        macroLayer.eachLayer(layer => {
            if (layer === targetLayer) {
                layer.setStyle({ color: '#ef4444', weight: 4, fillOpacity: 0.4 });
                layer.bringToFront();
            } else {
        ...
        if (distritoLayer) {
            distritoLayer.eachLayer(l => l.setStyle({ color: '#10b981', weight: 1, fillOpacity: 0.1, dashArray: '3' }));
        }
        actualizarEtiquetas();
    } else {
...
function setupLayerControls() {
    ...
    opacityOTB.addEventListener('input', function() {
        ...
        if (otbsLayer && otbVisible) {
            otbsLayer.setStyle({ fillOpacity: val / 100 * 0.5, opacity: val / 100 * 0.8 });
        }
    });
...
function buildOTBLayer(macro, distrito) {
    ...
    otbsLayer = L.geoJSON(features, {
        style: { color: '#10b981', weight: 2, fillOpacity: 0.1 * opacity, opacity: 0.8 * opacity },
        onEachFeature: (feature, layer) => {
            ...
            // ✅ AHORA SOLO AL HOVER (sticky tooltip)
            layer.bindTooltip(nombre, {
                permanent: false,
                sticky: true,
                direction: 'auto',
            ...
            layer.on('click', () => {
                resetOTBHighlight();
                layer.setStyle({ color: '#f59e0b', weight: 4, fillOpacity: 0.3 });
                layer.bringToFront();
                layer.openPopup();
...
function resetOTBHighlight() {
    ...
    const opacity = document.getElementById('opacity-otb').value / 100;
    otbsLayer.eachLayer(layer => {
        layer.setStyle({ color: '#10b981', weight: 2, fillOpacity: 0.1 * opacity, opacity: 0.8 * opacity });
        layer.closePopup();
        layer.closeTooltip();
    });
    actualizarEtiquetas();
